...
describe MoviesController do
  describe 'add director' do
    before :each do
      @m=mock_model(Movie, :title => "Star Wars", :director => "director", :id => "1")
      Movie.stub_model!(:find).with("1").and_return(@m)
    end
    it 'should call update_attributes and redirect' do
      @m.stub_model!(:update_attributes!).and_return(true)
  ...
  describe 'happy path' do
    ...
    it 'should select the Similar template for rendering and make results available' do
      Movie.stub_model!(:similar_directors).with('director').and_return(@m)
      get :similar, :movie_id => "1"
  ...
  describe 'sad path' do
    before :each do
      m=mock_model(Movie, :title => "Star Wars", :director => nil, :id => "1")
      Movie.stub_model!(:find).with("1").and_return(m)
    end
  ...
  describe 'create and destroy' do
    it 'should create a new movie' do
      MoviesController.stub_model(:create).and_return(mock_model('Movie'))
      post :create, {:id => "1"}
    end
    it 'should destroy a movie' do
      m = mock_model(Movie, :id => "10", :title => "blah", :director => nil)
      Movie.stub_model!(:find).with("10").and_return(m)
